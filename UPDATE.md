...
同，网友都说vue比react容易，说JSX语法难懂，但不知道怎的我觉得JSX的语法更加清晰，感觉vue反而复杂一点。有的地方还不是
...
scrollBehavier，但看了很久，都不是很明白他到底能不能完成我这个需求，然后网上查了一下，基本是说使用scrollBehavior方法的，
没什么帮助。因此另辟途径，自己尝试实现。
...
### 1.25
```
    看到之前写的面包屑导航组件的逻辑，当初的想法是根据路径去判断当前页，思路是将当前路径根据'/'分割成数组，遍历时将后一项
添加前一项，然后在组件里创建一个对象保存键值对，将路径和导航展示的内容绑定在一起。现在看其实这个思路不太好，首先根据路径
去判断的话，依赖性太强，如果路由更改了路径得重新修改；第二，将路径与导航标签用键值对保存的话其实耦合也是很强的，自从加入
动态改变分类项目之后，这种做法就有点死板了。
    
    于是乎就抽时间重做了这个组件，鉴于路由能传参，所以想根据路由对象传递的参数去判断当前的位置。思路是在路由表中固定不变
的项目里，将要显示在面包屑导航上的名称写入meta里面，然后遍历路由的matched数组。这个数组能获取到当前嵌套的路径，因此可以得
每层路由的父子关系，然后动态路由项，就取出store中从数据库里获取category数据做对比，返回与当前params里传递的categoryName
一致的条目。其中有个问题就是，当初我写路由表时，前后台的详情页和列表页之间是兄弟关系，因此获取详情页时，是获取不到列表页
路由携带的meta信息。经过多番思考，决定重构路由表，将详情页做成列表页的子路由，然后这样就能获取层级关系。

    整个实现都比较顺利，但也是出现了问题。后台的详情页嵌套到列表页，路径是':id'，本身是打算渲染列表页时，如果当前路由名称
不等于详情页组件的路由名就不渲染，这个逻辑在前台是没有问题的，但后台登录后是默认跳转到列表页，而我的列表页本身是绑定了别
名为admin即后台主页的，这样登录就会自带了id属性，传入了详情页并发起请求，最终导致详情页因为获取不到文章而跳转到404组件。

    多番修改及尝试后，终于被我发现原因。其实不单单是首页跳转跳转详情页，如果在其他后台非详情页页面刷新后，登录进去时都会
携带该组件的id值，因此总是会发生错误。但只要将详情页的路径指向修改一下就没有问题，最后我是修改成'admin/posts/:id'，这样
就必须要再详情页跳转并携带id值时才会载入这个组件，虽然这样有点硬编码的形式，但确实问题时解决了，暂时也想不到更好的办法，
先凑合用着。
```
### 1.30
```
    这几天都在看其他的文章，在学习nuxt时了解了vue-meta，现在我设置的页面标题时beforeEach时获取to的meta里的title项去设置，
没有固定的则再根究面包屑导航的计算获取，详情页则获取到文章标题后再设置。感觉这个库很适合，但感觉其他的功能都用不上，想想
还要给多个页面再重新添加meta就算了，暂时先不加入。

    经过这几天的学习，对meta有了更深刻的了解。想当初在写页面跳转滑动时看过的scrollBehavior方法，当时以为不能做锚点跳转，
只能根据提供的x和y值跳转到特定位置，但其实现在再仔细看看，文档里有一处时说可以根据meta做更细粒度的跳转操作，除了meta，其
实也可以用params做判断的，只要目标组件的页面中有那个锚点，自然就能跳转后滑动到锚点。当然对于我的需求还是不适合，因为我的
评论详情页跳转时锚点还不存在的，是在组件异步获取到评论列表时刷新后才加载，因此这个方法还是不适合，但这让我对这个方法的使
用有了更深的理解。

    随后发现当后台博文详情页里的评论如果太多，需要分页显示的时候，在评论列表跳转会由于分页的缘故，导致部分的评论是无法跳
转的。为此我在屏幕滑动组件做了修改，将分页的当前页、当前显示的分页列表以及分页的更改函数传入到该滑动组件中，当路由携带着
锚点信息加载组件，而组件又在当前的列表中无法获取锚点时，根据传入的当前页码和更改函数进行翻页，知道找到为止，之后就会滑动
到对应的项目了。
```