...
##### 1.18
```
    这几天都不是很认真去做demo，都是做一会又走去弄其他事了，进度有点慢。
    
    后台的展示主要是通过表格做展示，但问题是因为前台覆盖了变量，现在后台的表格样式也是跟前台的一样的，如果改的话，难道要
像之前一样，在每个组件里做更改吗，有没有更高效的方法，例如从css中直接控制表格的样式，毕竟即使组件能通过属性设定，但根本上
来说也是通过css去更改样式的，因此一定有对应的类名。
...
border去掉。
    ...
    虽然听着很简单，但也是经过了很长时间反复去修改才得到的，这个坑算是踩过去了。
    
    之后把前台categories组件的获取合并到DataDisplayHOC里去，本来是想减少点重复代码。但后来偶然间发现了，这个做法会在每次
切换跳转路由获取数据时，都会重新创建这个高阶组件的实例，对于边栏来说，实在没必要，毕竟获取的数据量和付出的额外运行量是反
比的，决定回滚到之前的操作，后台的页面和基本ui逻辑已经完成，之后在加入编辑器，还有部分操作数据库的操作，demo基本就完成了，
不过未来几天都不怎么在家，估计得拖到下周了。
```